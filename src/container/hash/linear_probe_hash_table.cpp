...
// template <typename KeyType, typename ValueType, typename KeyComparator>
// bool HASH_TABLE_TYPE::IterateTable(Transaction *transaction, const KeyType &key, const ValueType &value, std::vector<size_t> *result, int flag){
// 	size_t hash_value = hash_fn_.GetHash(key);
// 	/*Table slot index determines the block page the <key,value> pair should go into */
// 	size_t table_slot_index = hash_value%num_slots_;
// 	page_id_t block_page_id = header_page_->GetBlockPageId(table_slot_index);
...
// 	result.push_back(0);
// 	while(true){
// 		/*Flag -> 0: Check if slot is empty
// 		Flag -> 1: Check if <k,v> is present in the slots
// 		Flag ->2: Check if key is present in the slot*/
// 		if(flag == 0){
// 			if(!(block_page->IsOccupied(page_slot_index)) || !(block_page->IsReadable(page_slot_index)){
// 				result.push_back(table_slot_index);
// 				result.push_back(page_slot_index);
// 				return true;
// 			}
//
// 			else if(!(comparator_(block_page->KeyAt(page_slot_index),key)) && block_page->ValueAt(page_slot_index) == value){
